...
import { useAuth } from "../../../hooks/useAuth";  // Corrigido para importar de onde é o hook
...
export default function Etapa1() {
  ...
  const { updateAuth } = useAuth();  // Usando o hook useAuth para acessar o método updateAuth
  const navigation = useNavigation(); 

  function testandoDate() {
    if (dia && mes && ano) {
      const date = new Date(ano, mes - 1, dia);
      const idade = new Date().getFullYear() - date.getFullYear(); // Calculando a idade
      updateAuth({ idade });  // Atualizando a idade no contexto global
      navigation.navigate("Etapa2"); // Navegando para a próxima etapa
    } else {
